 ...
 */
if ( ! defined( 'ABSPATH' ) ) {
    $error = __('ABSPATH is not defined', RW_PLUGIN_NAME);
    exit ($error); // Exit if accessed directly
}
...
function wpcf7_datetime_form_tag_handler( $tag ) {
	...
	*/
    ...
    // jQuery validate
        ...
        $atts['data-validation-help'] = __('Укажите дату и время в формате: 00.00.0000 00:00 или воспользуйтесь календарем', RW_THEME_NAME);
...
function wpcf7_add_tag_generator_datetime() {
	...
	$tag_generator->add( 'datetimepicker', __( 'date and time picker', 'contact-form-7' ), 'wpcf7_tag_generator_datetime' );
}
...
function add_datetimepicker()
{
   ...
   # $dir_validate_css = '//cdnjs.cloudflare.com/ajax/libs/jquery-form-validator/2.3.26/theme-default.min.css';
    ...
    // global register  
    # wp_register_script('jquery.form-validator', $dir_validate_js, array(), '2.3.26', true );// inc jQuery validator
    wp_register_script( 'jquery.datetimepicker', $dir_js, array(), '1.3.4', true );// inc datetime  
	// call
    wp_enqueue_style( 'jquery.datetimepicker', $dir_css );// inc css
...
jQuery.validate({
  ...
  lang : "ru", // set locale
  modules : "html5", //add support html5  
});
' );
